...
reload_upnpd() {
	...
	if [ 1 -eq "$test" ]; then
		...
		[ -f /etc/init.d/miniupnpd ] && {
			...
			log_enable=$(uci get upnpd.config.log_output)
			[ "$log_enable" -eq 1 ] && echo "<b> The page does not refresh. Please click on the required configuration page.</b>"
		}
		[ -f /etc/init.d/upnpd ] && {
...
for ucifile in $(ls /tmp/.uci/* 2>&-); do
	...
	config_allclear
	equal "$ucifile" "/tmp/.uci/network" && {
	  config_load "/etc/config/$package"
#	  wan_config=$(grep -v "network.wan=interface" /tmp/.uci/network | grep -e wan -e gateway)
#	  lan_config=$(grep -v "network.lan=interface" /tmp/.uci/network | grep lan)
	  nw_iface=$(cat /tmp/.uci/network | grep "\-network\." | cut -d'.' -f 2)
	  nw_iface_param=$(cat /tmp/.uci/network | grep "\-network\." | cut -d'.' -f 3)
#	  echo "nw_iface=<$nw_iface> nw_iface_param=<$nw_iface_param>"  >>/test1
	  [ -n "$nw_iface" -a -z "$nw_iface_param"] &&
          {
	    echo "ifdown $nw_iface" >>/test1
	    ifdown $nw_iface
          }
	  cat /tmp/.uci/network | awk -F[.,=] '/interface/ { print "uci set network." $2 ".flag=1" }' |sh
#	  [ -z "$wan_config" ] &&  {
#	    uci set network.wan.flag=0
#	  }
#	  [ -z "$lan_config" ] &&  {
#	    uci set network.lan.flag=0
#	  }
	  config_allclear
        }
	...
	case "$package" in
		...
		"network")
			...
			if [ "$flag" = "1" ] ; then
			  ...
			  config_load "/etc/config/vap0"
			  config_load "/etc/config/vap1"
			  config_load "/etc/config/vap2"
			  config_load "/etc/config/vap3"
			  config_load "/etc/config/vap4"
			  config_load "/etc/config/vap5"
			  config_load "/etc/config/vap6"
			  config_load "/etc/config/vap7"
			  /sbin/wifi up 
			...
			ifaces=$(uci show network | grep "=interface"  | cut -d'.' -f 2 | cut -d'=' -f 1)
